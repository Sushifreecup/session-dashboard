...
export default function SessionsPage() {
  ...
  const identifyAccount = (sessionCookies: Cookie[], fallbackId: string): AccountInfo => {
    ...
    if (domainStr.includes("openart.ai") || domainStr.includes("nijijourney"))
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    {session.user_agent && (
                      <div className="flex items-center gap-1.5 text-xs text-blue-400/60 bg-blue-400/5 px-2 py-1 rounded-lg border border-blue-400/10">
                        <UAIcon size={12} />
                        <span className="font-medium truncate max-w-[80px]">CAPTURED UA</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div className="flex items-center gap-5">
                      <div className={`p-5 rounded-3xl bg-white/5 ${accountMap[selectedSession.id]?.color}`}>
                        {accountMap[selectedSession.id]?.icon && React.createElement(accountMap[selectedSession.id].icon, { size: 56 })}
                      </div>
                      <div>
                        <h3 className="text-3xl font-bold">{accountMap[selectedSession.id]?.identifier}</h3>
                        <p className="text-gray-400">Captured: {new Date(selectedSession.captured_at).toLocaleString()}</p>
                      </div>
                    </div>
